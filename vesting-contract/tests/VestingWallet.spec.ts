...
const CONFIG_ADDRESS = Address.parse(
...
describe("VestingWallet", () => {
  ...
  beforeEach(async () => {
    ...
    ownerKeyPair = keyPairFromSeed(
      Buffer.from(base64Decode("vt58J2v6FaBuXFGcyGtqT5elpVxcZ+I1zgu/GUfA5uY="))
    );
    notOwnerKeyPair = keyPairFromSeed(
      Buffer.from(base64Decode("vt59J2v6FaBuXFGcyGtqT5elpVxcZ+I1zgu/GUfA5uY="))
    );
    ...
    const deployResult = await vestingWallet.sendDeploy(
      deployer.getSender(),
      toNano("0.05")
    );
  ...
  async function checkLockupData(expectedSeqno?: number) {
    ...
    expect(lockupData.vestingSenderAddress.toString()).toBe(
      vestingSender.address.toString()
    );
  ...
  async function addWhitelist(
    count: number,
    increaser: SandboxContract<TreasuryContract>
  ) {
    ...
    expect(result.transactions).toHaveTransaction({
      ...
      success: true,
      body: beginCell()
        .storeUint(Opcodes.add_whitelist_response, 32)
        .storeUint(123 + count, 64)
        .endCell(),
    });
    ...
    expect(lastTx.inMessage.info.src.toString()).toBe(
      vestingWallet.address.toString()
    );
    expect(lastTx.inMessage.info.dest.toString()).toBe(
      increaser.address.toString()
    );
    expect(lastTx.inMessage.info.value.coins).toBeLessThan(toNano("1"));

    const whitelist = await vestingWallet.getWhitelist();
    const whitelistStrings = whitelist.map((address: Address) =>
      address.toString()
    );
    ...
    for (let i = 0; i < count; i++) {
      expect(
        whitelistStrings.indexOf(addresses[i].toString()) > -1
      ).toBeTruthy();
    ...
    expect(
      await vestingWallet.getIsWhitelisted(
        Address.parse(
          "0:0073f6ed7a84ac7d90739db7741f9d487478854b69960769f74859081e592d1c"
        )
      )
    ).toBeFalsy();
  ...
  async function whitelistNotVestingSender(
    notVestingSender: SandboxContract<TreasuryContract>
  ) {
    const whitelistBefore = await vestingWallet.getWhitelist();
    expect(whitelistBefore.length).toBe(0);

    const result = await vestingWallet.sendAddWhitelist(
      notVestingSender.getSender(),
      {
        queryId: 123,
        value: toNano("1"),
  ...
  it("add whitelist twice", async () => {
    ...
    const result2 = await vestingWallet.sendAddWhitelist(
      increaser.getSender(),
      {
        queryId: 777,
        value: toNano("1"),
    ...
    const whitelistStrings2 = whitelist2.map((address: Address) =>
      address.toString()
    );
    expect(whitelist2.length).toBe(3);

    expect(
      whitelistStrings2.indexOf(addresses[0].toString()) > -1
    ).toBeTruthy();
    expect(
      whitelistStrings2.indexOf(addresses[1].toString()) > -1
    ).toBeTruthy();
    expect(
      whitelistStrings2.indexOf(addresses[4].toString()) > -1
    ).toBeTruthy();
    expect(await vestingWallet.getIsWhitelisted(addresses[0])).toBeTruthy();
    expect(await vestingWallet.getIsWhitelisted(addresses[1])).toBeTruthy();
  ...
  it("get_unlocked_amount", async () => {
    expect(await vestingWallet.getLockedAmount(VESTING_START_TIME - 100)).toBe(
      toNano(123000n)
    );
    expect(await vestingWallet.getLockedAmount(VESTING_START_TIME)).toBe(
      toNano(123000n)
    );
    expect(
      await vestingWallet.getLockedAmount(VESTING_START_TIME + UNLOCK_PERIOD)
    ).toBe(toNano(123000n));
    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + CLIFF_DURATION - 1
      )
    ).toBe(toNano(123000n));

    expect(
      await vestingWallet.getLockedAmount(VESTING_START_TIME + CLIFF_DURATION)
    ).toBe(toNano(123000n - (123000n * 2n) / 12n));

    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 3 - 1
      )
    ).toBe(toNano(123000n - (123000n * 2n) / 12n));
    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 3
      )
    ).toBe(toNano(123000n - (123000n * 3n) / 12n));
    ...
    ).toBe(toNano(123000n - (123000n * 4n) / 12n));

    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 7 - 1
      )
    ).toBe(toNano(123000n - (123000n * 6n) / 12n));
    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 7
      )
    ).toBe(toNano(123000n - (123000n * 7n) / 12n));

    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 9 - 1
      )
    ).toBe(toNano(123000n - (123000n * 8n) / 12n));
    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 9
      )
    ).toBe(toNano(123000n - (123000n * 9n) / 12n));

    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 10 - 1
      )
    ).toBe(toNano(123000n - (123000n * 9n) / 12n));
    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 10
      )
    ).toBe(toNano(123000n - (123000n * 10n) / 12n));

    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 11 - 1
      )
    ).toBe(toNano(123000n - (123000n * 10n) / 12n));
    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 11
      )
    ).toBe(toNano(123000n - (123000n * 11n) / 12n));

    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 12 - 1
      )
    ).toBe(toNano(123000n - (123000n * 11n) / 12n));
    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + UNLOCK_PERIOD * 12
      )
    ).toBe(toNano(0n));

    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + VESTING_TOTAL_DURATION
      )
    ).toBe(toNano(0n));
  });
  ...
  it("get_unlocked_amount no cliff", async () => {
    ...
    expect(
      await vestingWallet.getLockedAmount(VESTING_START_TIME + UNLOCK_PERIOD)
    ).toBe(toNano(123000n - (123000n * 1n) / 12n));
    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + CLIFF_DURATION - 1
      )
  ...
  it("get_unlocked_amount unlock = duration", async () => {
    ...
    expect(
      await vestingWallet.getLockedAmount(
        VESTING_START_TIME + VESTING_TOTAL_DURATION / 2
      )
    ).toBe(toNano(123000n));
    expect(
      await vestingWallet.getLockedAmount(
  ...
  it("get_unlocked_amount total_duration == unlock_period", async () => {
    ...
    for (let i = 0; i <= 11; i++) {
      expect(
        await vestingWallet.getLockedAmount(
          VESTING_START_TIME + UNLOCK_PERIOD * i
        )
      ).toBe(toNano(123000n));
    }
  ...
  async function transferSuccess(time: number, value: bigint) {
    ...
    expect(result.transactions).toHaveTransaction({
      ...
      to: owner.address,
      body: beginCell()
        .storeUint(Opcodes.send_response, 32)
        .storeUint(567, 64)
        .endCell(),
    });
  ...
  async function transferNotOwner(sender: SandboxContract<TreasuryContract>) {
    ...
    const result = await vestingWallet.sendInternalTransfer(
      sender.getSender(),
      {
        value: toNano("1"),
        queryId: 567,
    ...
    if (comment) {
      body =
        typeof comment === "string"
          ? beginCell()
              .storeUint(0, 32)
              .storeStringTail(comment as string)
              .endCell()
          : beginCell()
              .storeUint(comment as number, 32)
              .storeUint(0x456, 64)
              .endCell();
    }
  ...
  async function transferAllowElector(
    time: number,
    sender: SandboxContract<TreasuryContract>,
    to: Address,
    isWhitelist: boolean,
    ...
    expect(bouncedTx.inMessage.info.dest.toString()).toBe(
      vestingWallet.address.toString()
    );
  ...
  it("if locked && to anyone - senmode != 3 rejected", async () => {
    const notVestingSender = await blockchain.treasury("notVestingSender");
    await transferReject(
      ErrorCodes.send_mode_not_allowed,
      owner,
      notVestingSender.address,
      false,
      1,
  ...
  it("if locked && whitelist - non-bounceable rejected", async () => {
    const notVestingSender = await blockchain.treasury("notVestingSender");
    await transferReject(
      ErrorCodes.non_bounceable_not_allowed,
      owner,
      notVestingSender.address,
      true,
      3,
  ...
  it("if locked && whitelist - stateInit rejected", async () => {
    const notVestingSender = await blockchain.treasury("notVestingSender");
    await transferReject(
      ErrorCodes.state_init_not_allowed,
      owner,
      notVestingSender.address,
      true,
      3,
  ...
  it("if locked && whitelist - bin comment rejected", async () => {
    const notVestingSender = await blockchain.treasury("notVestingSender");
    await transferReject(
      ErrorCodes.comment_not_allowed,
      owner,
      notVestingSender.address,
      true,
      3,
  ...
  it("if locked && whitelist - 0x1000 allowed", async () => {
    ...
    await transferAllow(
      ...
      true,
      false,
      Opcodes.single_nominator_pool_withdraw
    );
  });
  it("if locked && whitelist - 0x1001 allowed", async () => {
    ...
    await transferAllow(
      ...
      true,
      false,
      Opcodes.single_nominator_pool_change_validator
    );
  });
  ...
  it('if locked && whitelist - "y" rejected', async () => {
    const notVestingSender = await blockchain.treasury("notVestingSender");
    await transferReject(
      ErrorCodes.symbols_not_allowed,
      owner,
      notVestingSender.address,
      true,
      3,
